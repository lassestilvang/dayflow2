...
test.describe("Calendar View Features", () => {
  ...
  test.describe("Week Display", () => {
    ...
    test("displays 7 days in week view", async ({ page }) => {
      const dayHeaders = page.locator('[data-testid="day-header"], .day-header, [class*="day"]');
  ...
  test.describe("Current Day Highlighting", () => {
    test("current day is visually distinct", async ({ page }) => {
      const currentDay = page.locator('[class*="current"], [class*="today"], [aria-current="date"]');
  ...
  test.describe("Current Time Indicator", () => {
    test("current time indicator is displayed during business hours", async ({ page }) => {
      const now = new Date();
      const hour = now.getHours();
      
      if (hour >= 6 && hour <= 22) {
        const timeIndicator = page.locator('[class*="time-indicator"], [class*="current-time"]');
  ...
  test.describe("Time Slots", () => {
    ...
    test("time slots are clickable", async ({ page }) => {
      const timeSlot = page.locator('[class*="time-slot"], [data-testid="time-slot"]').first();
  ...
  test.describe("Event Rendering", () => {
    test("events display at correct times", async ({ page }) => {
      const eventBlocks = page.locator('[class*="event"], [data-testid*="event"]');
  ...
  test.describe("Calendar Navigation", () => {
    test("can navigate through weeks", async ({ page }) => {
      const initialHeader = await page.locator('[class*="date-header"]').first().textContent();
      
      await goToNextWeek(page);
      const nextHeader = await page.locator('[class*="date-header"]').first().textContent();
      
      expect(nextHeader).not.toBe(initialHeader);
      
      await goToPreviousWeek(page);
      const backHeader = await page.locator('[class*="date-header"]').first().textContent();
    ...
    test("calendar maintains scroll position after navigation", async ({ page }) => {
      const calendarContent = page.locator('[class*="calendar-content"]').first();
  ...
  test.describe("Scrolling Behavior", () => {
    ...
    test("can scroll to different times of day", async ({ page }) => {
      const scrollContainer = page.locator('[class*="calendar-content"]').first();
