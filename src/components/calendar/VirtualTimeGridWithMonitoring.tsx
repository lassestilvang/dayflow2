...
import { useVirtualScrollMonitor } from "@/hooks/usePerformanceMonitor";
import {
  getHourSlots,
...
interface VirtualTimeGridWithMonitoringProps {
  date: Date;
...
export const VirtualTimeGridWithMonitoring = React.memo(function VirtualTimeGridWithMonitoring({
  date,
  timeBlocks,
  onTimeSlotClick,
  onBlockClick,
...
}: VirtualTimeGridWithMonitoringProps) {
  ...
  const performanceMonitor = useVirtualScrollMonitor({
    enabled: enablePerformanceMonitoring,
    componentName: `VirtualTimeGrid-${date.toISOString().split('T')[0]}`,
    trackDomNodes: true,
  });
  ...
  return (
    <div className="relative flex flex-col" style={{ minWidth: "200px", width: "200px" }}>
      ...
      {/* Performance monitoring info (only in development) */}
      {enablePerformanceMonitoring && process.env.NODE_ENV === 'development' && (
        <div className="absolute top-2 left-2 z-10 bg-black/80 text-white text-xs p-2 rounded">
          <div>Render: {Math.round(performanceMonitor.metrics.renderTime)}ms</div>
          <div>Visible: {visibleSlots.length}/{hourSlots.length}</div>
