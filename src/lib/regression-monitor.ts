...
class RegressionMonitor {
  ...
  private analyzeRegressions(current: BaselineMetrics): void {
    ...
    if (current.frameRate.average < previous.frameRate.average * 0.9) { // 10% drop
      alerts.push(this.createAlert('regression', 'Frame Rate', previous.frameRate.average, current.frameRate.average, this.config.frameRateThreshold));
    }

    // Check memory usage regression
    if (current.memoryUsage.increaseMB > previous.memoryUsage.increaseMB * 1.2) { // 20% increase
      alerts.push(this.createAlert('regression', 'Memory Usage', previous.memoryUsage.increaseMB, current.memoryUsage.increaseMB, this.config.memoryThresholdMB));
    }

    // Check collision detection regression
    if (current.collisionDetection.averageTime > previous.collisionDetection.averageTime * 1.5) { // 50% slower
      alerts.push(this.createAlert('regression', 'Collision Detection', previous.collisionDetection.averageTime, current.collisionDetection.averageTime, this.config.collisionTimeThreshold));
    }

    // Check store update frequency regression
    if (current.storeUpdates.frequency > previous.storeUpdates.frequency * 1.3) { // 30% increase
      alerts.push(this.createAlert('regression', 'Store Updates', previous.storeUpdates.frequency, current.storeUpdates.frequency, this.config.storeUpdateThreshold));
    }

    // Check threshold violations
    if (current.frameRate.average < this.config.frameRateThreshold) {
      alerts.push(this.createAlert('threshold_exceeded', 'Frame Rate', previous.frameRate.average, current.frameRate.average, this.config.frameRateThreshold));
    }

    if (current.memoryUsage.increaseMB > this.config.memoryThresholdMB) {
      alerts.push(this.createAlert('threshold_exceeded', 'Memory Usage', previous.memoryUsage.increaseMB, current.memoryUsage.increaseMB, this.config.memoryThresholdMB));
    }

    if (current.collisionDetection.averageTime > this.config.collisionTimeThreshold) {
      alerts.push(this.createAlert('threshold_exceeded', 'Collision Detection', previous.collisionDetection.averageTime, current.collisionDetection.averageTime, this.config.collisionTimeThreshold));
    }

    if (current.storeUpdates.frequency > this.config.storeUpdateThreshold) {
      alerts.push(this.createAlert('threshold_exceeded', 'Store Updates', previous.storeUpdates.frequency, current.storeUpdates.frequency, this.config.storeUpdateThreshold));
    }
  ...
  private createAlert(
    ...
    return {
      ...
      threshold,
      severity,
      description: this.generateAlertDescription(type, metric, previousValue, currentValue, change),
    };
  }

  private calculateSeverity(metric: string, changePercent: number): 'low' | 'medium' | 'high' | 'critical' {
  ...
  private generateAlertDescription(
    ...
    switch (type) {
      ...
      case 'threshold_exceeded':
        return `${metric} exceeded threshold: ${currentValue.toFixed(2)} (threshold: ${this.getThresholdForMetric(metric)})`;
      default:
  ...
  private getThresholdForMetric(metric: string): number {
    switch (metric) {
      case 'Frame Rate': return this.config.frameRateThreshold;
      case 'Memory Usage': return this.config.memoryThresholdMB;
      case 'Collision Detection': return this.config.collisionTimeThreshold;
      case 'Store Updates': return this.config.storeUpdateThreshold;
      default: return 0;
    }
  }
  ...
  private async sendAlert(alert: RegressionAlert): Promise<void> {
    ...
    console.warn(`[REGRESSION ALERT] ${alert.severity.toUpperCase()}: ${alert.description}`);
  ...
  private persistAlerts(): void {
    try {
      localStorage.setItem('drag-drop-regression-alerts', JSON.stringify(this.alerts.slice(-50))); // Keep last 50 alerts
    } catch (error) {
  ...
  loadPersistedAlerts(): void {
    try {
      ...
      if (stored) {
        ...
        console.log(`[REGRESSION] Loaded ${this.alerts.length} persisted alerts`);
      }
    } catch (error) {
  ...
  getMetricsTrend(metric: keyof BaselineMetrics, window: number = 10): 'improving' | 'degrading' | 'stable' {
  ...
  private extractMetricValue(metrics: BaselineMetrics, metric: keyof BaselineMetrics): number {
    switch (metric) {
      case 'frameRate': return metrics.frameRate.average;
      case 'memoryUsage': return metrics.memoryUsage.increaseMB;
      case 'collisionDetection': return metrics.collisionDetection.averageTime;
      case 'storeUpdates': return metrics.storeUpdates.frequency;
      case 'timeSlotCreation': return metrics.timeSlotCreation.count;
      case 'dragOperations': return metrics.dragOperations.averageDuration;
      default: return 0;
    }
  }
  ...
  generateRegressionReport(): string {
    ...
    if (recentAlerts.length > 0) {
      ...
      recentAlerts.slice(0, 10).forEach(alert => {
        ...
        report += `${time} [${alert.severity.toUpperCase()}] ${alert.description}\n`;
      });
      report += '\n';
    }

    report += 'METRIC TRENDS:\n';
    const metrics: (keyof BaselineMetrics)[] = ['frameRate', 'memoryUsage', 'collisionDetection', 'storeUpdates'];
